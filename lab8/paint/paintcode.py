...
def not_dots():
    if ACTIVE_TOOL == 'rectangle' and current_figure != None:
        return current_figure.w > 1 and current_figure.h > 1
    elif ACTIVE_TOOL == 'circle' and current_figure != None:
        return current_figure.r > 2
    else:
        return False
...
def fill_size_picker(bg_color, border=False, border_color=None):
    ...
    def size_picker_draw_line(x1, x2, y, width): pygame.draw.line(main_size_picker.surf, color['black'], (x1, y),
                                                                  (x2, y), width)
...
def bfs_fill(origin_color, fill_color, x, y):
    ...
    def step(x, y):
        if x >= 0 and y >= 0 and x < surf_for_draw[0].w and y < surf_for_draw[0].h and surf_for_draw[0].surface.get_at(
                (x, y)) == origin_color:
            surf_for_draw[0].surface.set_at((x, y), fill_color)
            queue.append((x, y))
...
while run:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if event.button == 1 and surf_for_draw[1].rect.collidepoint(
                    event.pos) and stage_of_figure % 5 == 0 and not (
                    surf_change_by_diagonal or surf_change_by_right or surf_change_by_bottom or CHANGE_COLOR):
            ...
            if check_type != None:
                ...
                if check_type == ACTIVE_TOOL:
                    ...
                    ACTIVE_SIZE = button.determine_size_by_order(active_size_order, ACTIVE_TOOL)
                    button.fill_button(figure.TOOL_BUTTONS[ACTIVE_TOOL], True, color['picked_button'],
                                       color['picked_button_border'])
            ...
            if event.button == 1 and (ACTIVE_TOOL == 'pen' or ACTIVE_TOOL == 'eraser') and not (
                    surf_change_by_diagonal or surf_change_by_right or surf_change_by_bottom):
                if ACTIVE_TOOL == 'pen':
                    drawing_pen = True
                elif ACTIVE_TOOL == 'eraser':
                    drawing_eraser = True
            ...
            if event.button == 1 and CHANGE_COLOR and not (
                    surf_for_draw[4].rect.collidepoint(event.pos) or current_color_button.check_colision(event.pos)):
                CHANGE_COLOR = False
        if event.type == pygame.MOUSEMOTION:
            ...
            if drawing_pen or drawing_eraser:
                ...
                if drawing_pen: pygame.draw.line(surf_for_draw[0].surface, current_color, (x1, y1), (x2, y2),
                                                 ACTIVE_SIZE)
                if drawing_eraser: pygame.draw.line(surf_for_draw[0].surface, color['white'], (x1, y1), (x2, y2),
                                                    ACTIVE_SIZE)
            if ACTIVE_TOOL == 'eraser':
