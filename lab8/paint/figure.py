...
class FIGURE:
    ...
    def update_surf_rect_by_start_end(self):
        if self.type == 'rectangle':
            ...
            self.rect = self.surf.get_rect(topleft=(self.x, self.y))  # check for/determine collides
        if self.type == 'circle':
            self.rect = pygame.rect.Rect((self.startx, self.starty),
                                         (abs(self.endx - self.startx), abs(self.endy - self.starty)))
            self.rect.topleft = (self.startx, self.starty)

    def update_buttons(self):
        # PLEASE DON'T CHANGE
        if self.type == 'rectangle':
            self.buttons['top_left'] = button.BUTTON(5, 5, self.x + self.border_size // 2 - 5 // 2,
                                                     self.y + self.border_size // 2 - 5 // 2)
            self.buttons['top'] = button.BUTTON(5, 5, self.x + self.w // 2 - 5 // 2,
                                                self.y + self.border_size // 2 - 5 // 2)
            self.buttons['top_right'] = button.BUTTON(5, 5, self.x + self.w - 3 - self.border_size // 2,
                                                      self.y + self.border_size // 2 - 5 // 2)
            self.buttons['left'] = button.BUTTON(5, 5, self.x + self.border_size // 2 - 5 // 2,
                                                 self.y + self.h // 2 - 5 // 2)
            self.buttons['bottom_left'] = button.BUTTON(5, 5, self.x + self.border_size // 2 - 5 // 2,
                                                        self.y + self.h - 3 - self.border_size // 2)
            self.buttons['bottom'] = button.BUTTON(5, 5, self.x + self.w // 2 - 5 // 2,
                                                   self.y + self.h - 3 - self.border_size // 2)
            self.buttons['bottom_right'] = button.BUTTON(5, 5, self.x + self.w - 3 - self.border_size // 2,
                                                         self.y + self.h - 3 - self.border_size // 2)
            self.buttons['right'] = button.BUTTON(5, 5, self.x + self.w - 3 - self.border_size // 2,
                                                  self.y + self.h // 2 - 5 // 2)
        if self.type == 'circle':
...
def check_for_figure_button(event, current_figure, h):
    ...
    if 'top' in current_figure.buttons.keys() and current_figure.buttons['top'].rect.collidepoint(
            point_in_working_surf):
    ...
    if 'bottom' in current_figure.buttons.keys() and current_figure.buttons['bottom'].rect.collidepoint(
            point_in_working_surf):
    ...
    if 'left' in current_figure.buttons.keys() and current_figure.buttons['left'].rect.collidepoint(
            point_in_working_surf):
        change_figure_bottom['left'] = True
    if 'right' in current_figure.buttons.keys() and current_figure.buttons['right'].rect.collidepoint(
            point_in_working_surf):
...
def change_figure_by_buttom(current_figure, points):
    ...
    if current_figure.type == 'circle':
        if change_figure_bottom['top_left']:
            ...
            current_figure.r = min(abs(current_figure.br[0] - current_figure.startx) // 2,
                                   abs(current_figure.br[1] - current_figure.starty) // 2)
        ...
        if change_figure_bottom['top_right']:
            ...
            current_figure.r = min(abs(current_figure.startx - current_figure.endx) // 2,
                                   abs(current_figure.starty - current_figure.endy) // 2)
...
def fill_bg_tools(except_type=None):
    [button.fill_button(tool, True, color['upper_block'], color['border_tools']) for type, tool in TOOL_BUTTONS.items()
     if type != except_type]
